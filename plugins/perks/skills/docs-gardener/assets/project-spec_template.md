# {{PROJECT_NAME}} Service Specification

Template usage: Replace all `{{...}}` placeholders with project-specific values before finalizing.
...
{{PROJECT_NAME}} is a long-running automation service that continuously reads work from an issue tracker
({{ISSUE_TRACKER_NAME}} in this specification version), creates an isolated workspace for each issue, and runs a
...
- {{PROJECT_NAME}} is a scheduler/runner and tracker reader.
...
{{PROJECT_NAME}} is easiest to port when kept in these layers:
...
5. `Integration Layer` ({{ISSUE_TRACKER_NAME}} adapter)
...
- Issue tracker API ({{ISSUE_TRACKER_NAME}} for `tracker.kind: {{issue_tracker_kind}}` in this specification version).
...
- `identifier` (string)
  - Human-readable ticket key (example: `{{PROJECT_KEY}}-123`).
- `title` (string)
...
- `kind` (string)
  ...
  - Current supported value: `{{issue_tracker_kind}}`
- `endpoint` (string)
  - Default for `tracker.kind == "{{issue_tracker_kind}}"`: `https://api.{{issue_tracker_kind}}.app/graphql`
- `api_key` (string)
  - May be a literal token or `$VAR_NAME`.
  - Canonical environment variable for `tracker.kind == "{{issue_tracker_kind}}"`: `{{ISSUE_TRACKER_API_KEY_ENV}}`.
  - If `$VAR_NAME` resolves to an empty string, treat the key as missing.
- `project_slug` (string)
  - Required for dispatch when `tracker.kind == "{{issue_tracker_kind}}"`.
...
For {{AGENT_RUNTIME_NAME}}-owned config values such as `approval_policy`, `thread_sandbox`, and
`turn_sandbox_policy`, supported values are defined by the targeted {{AGENT_RUNTIME_NAME}} app-server version.
Implementors should treat them as pass-through {{AGENT_RUNTIME_NAME}} config values rather than relying on a
hand-maintained enum in this spec. To inspect the installed {{AGENT_RUNTIME_NAME}} schema, run
`{{agent_runtime}} app-server generate-json-schema --out <dir>` and inspect the relevant definitions referenced
by `v2/ThreadStartParams.json` and `v2/TurnStartParams.json`. Implementations may validate these
fields locally if they want stricter startup checks.

- `command` (string shell command)
  - Default: `{{agent_runtime}} app-server`
  - The runtime launches this command via `bash -lc` in the workspace directory.
  - The launched process must speak a compatible app-server protocol over stdio.
- `approval_policy` ({{AGENT_RUNTIME_NAME}} `AskForApproval` value)
  - Default: implementation-defined.
- `thread_sandbox` ({{AGENT_RUNTIME_NAME}} `SandboxMode` value)
  - Default: implementation-defined.
- `turn_sandbox_policy` ({{AGENT_RUNTIME_NAME}} `SandboxPolicy` value)
  - Default: implementation-defined.
...
- If the workflow prompt body is empty, the runtime may use a minimal default prompt
  (`You are working on an issue from {{ISSUE_TRACKER_NAME}}.`).
...
- The software should attempt to adjust live behavior to the new config (for example polling
  cadence, concurrency limits, active/terminal states, {{agent_runtime}} settings, workspace paths/hooks, and
...
- `{{agent_runtime}}.command` is present and non-empty.

### 6.4 Config Fields Summary (Cheat Sheet)

This section is intentionally redundant so a coding agent can implement the config layer quickly.

- `tracker.kind`: string, required, currently `{{issue_tracker_kind}}`
- `tracker.endpoint`: string, default `https://api.{{issue_tracker_kind}}.app/graphql` when `tracker.kind={{issue_tracker_kind}}`
- `tracker.api_key`: string or `$VAR`, canonical env `{{ISSUE_TRACKER_API_KEY_ENV}}` when `tracker.kind={{issue_tracker_kind}}`
- `tracker.project_slug`: string, required when `tracker.kind={{issue_tracker_kind}}`
...
- `{{agent_runtime}}.command`: shell command string, default `{{agent_runtime}} app-server`
- `{{agent_runtime}}.approval_policy`: {{AGENT_RUNTIME_NAME}} `AskForApproval` value, default implementation-defined
- `{{agent_runtime}}.thread_sandbox`: {{AGENT_RUNTIME_NAME}} `SandboxMode` value, default implementation-defined
- `{{agent_runtime}}.turn_sandbox_policy`: {{AGENT_RUNTIME_NAME}} `SandboxPolicy` value, default implementation-defined
- `{{agent_runtime}}.turn_timeout_ms`: integer, default `3600000`
- `{{agent_runtime}}.read_timeout_ms`: integer, default `5000`
- `{{agent_runtime}}.stall_timeout_ms`: integer, default `300000`
...
- `{{AGENT_RUNTIME_NAME}} Update Event`
...
- For each running issue, compute `elapsed_ms` since:
  ...
  - `started_at`
- If `elapsed_ms > {{agent_runtime}}.stall_timeout_ms`, terminate the worker and queue a retry.
...
- Invocation: `bash -lc <{{agent_runtime}}.command>`
...
- The default command is `{{agent_runtime}} app-server`.
...
Reference: https://developers.openai.com/{{agent_runtime}}/app-server/
...
```json
{"id":1,"method":"initialize","params":{"clientInfo":{"name":"{{project_slug}}","version":"1.0"},"capabilities":{}}}
...
1. `initialize` request
   ...
   - If the targeted {{AGENT_RUNTIME_NAME}} app-server requires capability negotiation for dynamic tools, include the
...
3. `thread/start` request
   - Params include:
     ...
     - If optional client-side tools are implemented, include their advertised tool specs using the
       protocol mechanism supported by the targeted {{AGENT_RUNTIME_NAME}} app-server version.
...
- If implemented, supported tools should be advertised to the app-server session during startup
  using the protocol mechanism supported by the targeted {{AGENT_RUNTIME_NAME}} app-server version.
- Unsupported tool names should still return a failure result and continue the session.

`linear_graphql` extension contract:

- Purpose: execute a raw GraphQL query or mutation against {{ISSUE_TRACKER_NAME}} using {{PROJECT_NAME}}'s configured
  tracker auth for the current session.
- Availability: only meaningful when `tracker.kind == "{{issue_tracker_kind}}"` and valid {{ISSUE_TRACKER_NAME}} auth is configured.
...
- Reuse the configured {{ISSUE_TRACKER_NAME}} endpoint and auth from the active {{PROJECT_NAME}} workflow/runtime config; do
...
- `{{agent_runtime}}.read_timeout_ms`: request/response timeout during startup and sync requests
- `{{agent_runtime}}.turn_timeout_ms`: total turn stream timeout
- `{{agent_runtime}}.stall_timeout_ms`: enforced by orchestrator based on event inactivity
...
## 11. Issue Tracker Integration Contract ({{ISSUE_TRACKER_NAME}}-Compatible)
...
### 11.2 Query Semantics ({{ISSUE_TRACKER_NAME}})

{{ISSUE_TRACKER_NAME}}-specific requirements for `tracker.kind == "{{issue_tracker_kind}}"`:

- `tracker.kind == "{{issue_tracker_kind}}"`
- GraphQL endpoint (default `https://api.{{issue_tracker_kind}}.app/graphql`)
- Auth token sent in `Authorization` header
- `tracker.project_slug` maps to {{ISSUE_TRACKER_NAME}} project `slugId`
...
- {{ISSUE_TRACKER_NAME}} GraphQL schema details can drift. Keep query construction isolated and test the exact query
  fields/types required by this specification.

A non-{{ISSUE_TRACKER_NAME}} implementation may change transport details, but the normalized outputs must match the
...
{{PROJECT_NAME}} does not require first-class tracker write APIs in the orchestrator.
    ...
    ```json
    {
      ...
      "logs": {
        "codex_session_logs": [
          {
            "label": "latest",
            "path": "/var/log/{{project_slug}}/{{agent_runtime}}/MT-649/latest.log",
            "url": null
          }
        ]
      },
...
Running {{AGENT_RUNTIME_NAME}} agents against repositories, issue trackers, and other inputs that may contain
...
- Tightening {{AGENT_RUNTIME_NAME}} approval and sandbox settings described elsewhere in this specification instead
  of running with a maximally permissive configuration.
- Adding external isolation layers such as OS/container/VM sandboxing, network restrictions, or
  separate credentials beyond the built-in {{AGENT_RUNTIME_NAME}} policy controls.
- Filtering which {{ISSUE_TRACKER_NAME}} issues, projects, teams, labels, or other tracker sources are eligible for
...
- `tracker.kind` validation enforces currently supported kind (`{{issue_tracker_kind}}`)
- `tracker.api_key` works (including `$VAR` indirection)
- `$VAR` resolution works for tracker API key and path values
- `~` path expansion works
- `{{agent_runtime}}.command` is preserved as a shell command string
...
- {{ISSUE_TRACKER_NAME}} query uses the specified project filter field (`slugId`)
...
- Launch command uses workspace cwd and invokes `bash -lc <{{agent_runtime}}.command>`
- Startup handshake sends `initialize`, `initialized`, `thread/start`, `turn/start`
- `initialize` includes client identity/capabilities payload required by the targeted {{AGENT_RUNTIME_NAME}}
  app-server protocol
...
- If the optional `linear_graphql` client-side tool extension is implemented:
  ...
  - valid `query` / `variables` inputs execute against configured {{ISSUE_TRACKER_NAME}} auth
...
- A real tracker smoke test can be run with valid credentials supplied by `{{ISSUE_TRACKER_API_KEY_ENV}}` or a
...
- {{AGENT_RUNTIME_NAME}} launch command config (`{{agent_runtime}}.command`, default `{{agent_runtime}} app-server`)
...
- Optional `linear_graphql` client-side tool extension exposes raw {{ISSUE_TRACKER_NAME}} GraphQL access through the
  app-server session using configured {{PROJECT_NAME}} auth.
- TODO: Persist retry queue and session metadata across process restarts.
- TODO: Make observability settings configurable in workflow front matter without prescribing UI
  implementation details.
- TODO: Add first-class tracker write APIs (comments/state transitions) in the orchestrator instead
  of only via agent tools.
- TODO: Add pluggable issue tracker adapters beyond {{ISSUE_TRACKER_NAME}}.
