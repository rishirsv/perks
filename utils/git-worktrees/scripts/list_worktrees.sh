...
# Get worktree list (add trailing newline to ensure last entry is processed)
    ...
    if [[ $line == worktree* ]]; then
        ...
        if [ -n "$CURRENT_PATH" ]; then
            ...
            if [ "$CURRENT_PATH" = "$REPO_ROOT" ]; then
                ...
                ((FEATURE_COUNT++))
                FEATURE_WORKTREES+="${BLUE}[${FEATURE_COUNT}]${NC} ${CYAN}$CURRENT_BRANCH${NC}\n"
...
if [ -n "$FEATURE_WORKTREES" ]; then
    ...
    echo -e "${GRAY}No active worktrees (only main)${NC}"
    echo ""
fi
