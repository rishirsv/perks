...
## Phase 1.5: Behavior Lock (deep cleanup only)
...
8. **Needless abstraction**: pass-through wrappers, single-use helper layers, speculative indirection that adds a level without adding value
9. **Over-defensive code**: try/catch around code that can't throw, null checks on values that are never null, fallback defaults that can never trigger
10. **Verbose naming**: names that restate the type or context (\`userDataObject\`, \`handleOnClickEvent\`), or names padded with unnecessary qualifiers
