...
- `SOURCE_PATH`: /mnt/data/Project Dental_Report_25July2025_vS.pdf
- `EXTRACTION_STATUS`: complete
- `EXTRACTION_DATE`: 2026-02-17

## Required Source Evidence

- `SOURCE_TEXT_DIR`: extracted/verification/project-dental-2025-07-31/source-text/pdf
- `MONTAGE_DIR`: extracted/verification/project-dental-2025-07-31/montage
- `SOURCE_ARTIFACTS`:
  - `page-01.txt`
  - `page-02.txt`
  - `page-03.txt`
...
- `PROVENANCE_QA_JSON`: extracted/verification/project-dental-2025-07-31/qa/provenance.json
- `PROVENANCE_STATUS`: pass
- `GATES_QA_JSON`: extracted/verification/project-dental-2025-07-31/qa/gates.json
- `GATES_STATUS`: pass
- `SECTION_MAP_JSON`: extracted/verification/project-dental-2025-07-31/mapping/section-map.json
- `SECTION_ACCOUNTING_JSON`: extracted/verification/project-dental-2025-07-31/mapping/section-accounting.json
- `RENDER_TRACE_JSON`: extracted/verification/project-dental-2025-07-31/render/render-trace.json
...
| ------------------- | --------------------- | ------------------------------ | ---------------------------------------------------- | ----- |
| Executive Summary | 17 | `page-17.txt` | `# Executive Summary` bullets 1-10 | `[x]` |
| Business Overview | 4,5 | `page-04.txt`, `page-05.txt` | `# Business Overview` bullets 1-4 | `[x]` |
| Summary Financials | 6 | `page-06.txt` | `# Summary Financials` bullets 1-9 | `[x]` |
| Profit and Loss Overview / Financial Performance | 11,20,21 | `page-11.txt`, `page-20.txt`, `page-21.txt` | `# Profit and Loss Overview / Financial Performance` `## Overview` + `## Key Drivers` items 1-3 | `[x]` |
| Quality of Earnings | 22 | `page-22.txt` | `# Quality of Earnings` `## Overview` + `## Other considerations` bullets 1-1 | `Tables/charts excluded per extraction policy.` |
| Income Statement | 20,21 | `page-20.txt`, `page-21.txt` | `# Income Statement` bullets 1-6 | `Tables/charts excluded per extraction policy.` |
| Balance Sheet | 30,31 | `page-30.txt`, `page-31.txt` | `# Balance Sheet` bullets 1-6 | `Tables/charts excluded per extraction policy.` |
| Net Working Capital | 32,33,34,35 | `page-32.txt`, `page-33.txt`, `page-34.txt`, `page-35.txt` | `# Net Working Capital` `## Overview` + `## Net working capital adjustments` items 1-6 | `Tables/charts excluded per extraction policy.` |
| Net Debt (Cash) | 36 | `page-36.txt` | `# Net Debt (Cash)` `## Overview` + `## Net debt / cash adjustments` items 1-3 | `Table- or chart-based adjustment details were excluded per extraction policy.` |
| Industry Analysis | 13,14 | `page-13.txt`, `page-14.txt` | `# Industry Analysis` bullets 1-3 | `[x]` |
| <Addition Sections as Needed> | 8,9,18 | `page-08.txt`, `page-09.txt`, `page-18.txt` | `# <Addition Sections as Needed>` `## Value Creation`, `## Revenue drivers and cost levers (1/2)`, `## Valuation` | `Tables/charts excluded per extraction policy.` |
...
- Canadian dental market: $22B in size and only ~7%
  consolidated
- Dentalcorp’s market share is larger than DSO
  competitors 2-5 combined
- High revenue growth expected between 2024-2029
- Growing market due to (i) Canadians’ high “Dental I.Q.”,
  (ii) aging population, (iii) immigration, (iv) availability of
  government-funded dental coverage
- 91% recurring patient visits
- Primarily direct cash pay, neutral to negative working
  capital and limited exposure to insurance companies or
  government pay
- 70 - 75% of Canadians see a dentist every year
- Introduction of CDCP in 2024, a federally funded program
  designed to provide dental insurance coverage to
  uninsured Canadian residents who meet eligible criteria
- Scaled corporate infrastructure to support continued
  growth and drive operating leverage
...
- The Company’s selection criteria for a dental practice location includes:
  Acquisition Financial Thresholds
  •
  $2.0 million or more in revenue
  •
  $400,000 or more in EBITDA
  Practice-Level Attributes
  •
  Multiple dentists and hygienists for a diverse pool of revenue producers
  •
  Strong clinical reputation and practice standards
  •
  Attractive location and facility types
  Partner Dentist Demographics
  •
  Average age of a Partner Dentist at the time of acquisition of their practice is early 40s
  •
  Within Dentalcorp, the average age of all dentists is mid-40s
  Market Opportunity and Strategic Fit
  •
  Of the approximately 16,000 independently owned dental practices in Canada, management estimates that around 10,000
  meet the Company’s Partner Dentist acquisition criteria, with an additional 5,000 suitable for capacity utilization, patient
  chart acquisitions, or as supplementary locations for existing Partner Dentists—highlighting significant white space for
  continued growth
...
- The Company’s adjusted net working capital in FY24 is
...
- The last quarter frequently experiences the lowest
  NWC adjusted figures. This phenomenon can be
...
3. Net Finance Costs / Income tax recovery: Net Finance Costs: The major components of net finance costs are interest expense
...
Overview
The Quality of Earnings (“QofE”) analysis has been prepared
...
1. Table- or chart-based adjustment details were excluded per extraction policy.
2. Table- or chart-based adjustment details were excluded per extraction policy.
3. Table- or chart-based adjustment details were excluded per extraction policy.
...
## Revenue drivers and cost levers (1/2)
