...
function legalFooterLines(footerValues = {}) {
  ...
  const legalStructure =
    footerValues?.legalStructure || "limited liability partnership";
  const classification =
    footerValues?.documentClassification || "KPMG Confidential";
  return {
    legal1:
...
export function addBackCover(pptx, slideSpec = {}, ctx = {}) {
  ...
  const backCoverTokens =
    resolvedTheme.components?.[THEME_COMPONENT_KEYS.backCover] || {};
  ...
  if (!g.logoBox || !g.headingBox || !g.disclaimerBox || !g.urlBox) {
    throw new Error(
      'Missing required geometry for slide type "backCover" (logoBox/headingBox/disclaimerBox/urlBox)',
    );
  }

  // Prefer masters for the back-cover full-bleed gradient.
  if (!masterName) {
    addImageSmart(slide, gradientBackCover, {
      x: 0,
      y: 0,
      w: 13.333,
      h: 7.5,
  ...
  const defaultWebsiteText =
    backCoverTokens.defaultWebsiteText || "www.kpmg.com";
  ...
  slide.addText("The contacts at KPMG in connection with this report are:", {
    ...
    fontSize: Number(
      backCoverFontSizes.heading || resolvedTheme.typeSizes.title,
    ),
    bold: true,
    color: textColor,
    valign: "top",
  ...
  contacts.forEach((contact, idx) => {
    ...
    slide.addText(contact.name, {
      ...
      fontSize: toFiniteNumber(
        backCoverFontSizes.contactName,
        resolvedTheme.typeSizes.body,
      ),
      color: textColor,
      bold: true,
    ...
    slide.addText(`${contact.role}\n${contact.phone}\n${contact.email}`, {
      ...
      fontFace: bodyFont,
      fontSize: toFiniteNumber(
        backCoverFontSizes.contactDetails,
        resolvedTheme.typeSizes.body,
      ),
      color: textColor,
      wrap: true,
  ...
  slide.addText(disclaimer || legal.legal1, {
    ...
    fontFace: bodyFont,
    fontSize: toFiniteNumber(
      backCoverFontSizes.disclaimer,
      resolvedTheme.typeSizes.source,
    ),
    color: textColor,
    valign: "top",
    wrap: true,
  ...
  slide.addText(legal.classification, {
    ...
    fontFace: bodyFont,
    fontSize: toFiniteNumber(
      backCoverFontSizes.classification,
      resolvedTheme.typeSizes.source,
    ),
    color: textColor,
    bold: true,
