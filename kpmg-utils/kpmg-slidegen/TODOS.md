...
- [ ] Re-architecture audit follow-up (2026-03-09): make the new primitive-first and generated-runtime architecture authoritative end to end. Status 2026-03-09: the repo now has `templates-src/`, onboarding cases, codegen, and anti-drift docs, but several control points still allow source-of-truth drift, family-era bypasses, and CI false failures.
- [ ] Fix generated registry imports so promoted layouts load builders from the primitive's real `builderModule` instead of hardcoded `generator/builders/onboarded/*`. Status 2026-03-09: `scripts/codegen/generate-runtime-aggregates.mjs` emits imports that will not resolve for existing-primitive promotions or new primitive builders written under `generator/builders/primitives/`.
- [ ] Make built-in primitive authoring either truly authoritative or explicitly non-authoritative. Status 2026-03-09: `templates-src/kpmg-diligence/primitives/*.json` exists for built-ins, but `generator/runtime/slide-registry.js` still hardcodes builtin registry entries and codegen skips them, so editing builtin primitive fragments can silently fail to change runtime behavior.
- [ ] Remove generated-file inputs from codegen. Status 2026-03-09: `scripts/codegen/generate-runtime-aggregates.mjs` still reads `templates/kpmg-diligence/package/layouts.json` to preserve package metadata, leaving part of the source of truth in a generated file.
- [ ] Enforce promotion preflight against the full onboarding case lifecycle. Status 2026-03-09: `scripts/onboarding/case-lib.mjs` checks `qa.json`, `scorecard.json`, and `candidate.layout.json`, but it does not require the documented extract/classify/scaffold/render/compare artifact set or `record.status === 'compared'` before writing to `templates-src/`.
- [ ] Remove or hard-fail the retired `onboarding/layouts` and family-first onboarding entrypoints. Status 2026-03-09: `scripts/onboarding/init-layout.mjs` and family helpers in `scripts/onboarding/lib.mjs` still expose the old `source.json`/`seed/geometry.seed.json` path, which conflicts with the repo rule against fallback workflows.
- [ ] Expand `docs:verify` so it covers all onboarding-facing docs, not only the two canonical docs. Status 2026-03-09: stale references to `onboarding/layouts`, `family`, `source.json`, and unconditional `candidate.builder.js` still exist in `docs/onboarding/agent-batch-workflow.md`, `docs/onboarding/agent-review-checklists.md`, `README.md`, and `AGENTS.md`, but CI does not verify them.
- [ ] Make the skill bundle manifest deterministic or exclude volatile fields from cleanliness checks. Status 2026-03-09: `scripts/sync-skill-bundle.mjs` rewrites `skills/kpmg-slides/assets/bundle-manifest.json.generatedAt` on every sync, while CI uses `git diff --exit-code -- skills/kpmg-slides`, which makes the skill drift gate fail even when synced content is unchanged.
- [ ] Fix changed-layout CI detection so it uses the actual PR diff once, not duplicate local-style checks. Status 2026-03-09: `scripts/test-changed-layouts.mjs` reads `git diff --name-only HEAD`, which does not represent the PR delta on a clean CI checkout, and the same docs/changed-layout/primitive checks are then rerun inside `scripts/test-pr.mjs`.
- [ ] Align primitive and layout authoring schemas with the runtime geometry contract. Status 2026-03-09: `templates-src/kpmg-diligence/primitives/primitive.schema.json` allows geometry kinds the runtime rejects, and `templates-src/kpmg-diligence/layouts/layout-instance.schema.json` is still box-only despite runtime support for `number`, `string`, and `object` geometry kinds.
- [ ] Stop primitive scaffolding from routing through family policies. Status 2026-03-09: new primitive scaffolds still call `buildCandidateBuilderSource({ family })`, which means primitive reuse depends on `onboarding/policies/families.json` instead of directly reusing `basePrimitive.builderModule` and `basePrimitive.builderExport`.
- [ ] Make classification fail closed when no primitive matches. Status 2026-03-09: `scripts/onboarding/case-lib.mjs` persists the top primitive candidate even when all heuristic scores are `0`, which can silently choose an arbitrary base primitive instead of forcing an explicit authoring decision.
- [ ] Consolidate duplicate or layout-shaped primitives before the next onboarding batch. Status 2026-03-09: `divider`, `dividerDark`, and `dividerLight` should be one primitive plus layout/master overrides; `analysisWideChart2ColsText` and `analysisWideChartTableText` are strong merge candidates; `cover`, `backCover`, and `businessOverview` are still closer to legacy layout names than reusable rendering primitives.
- [ ] Add the next missing semantic primitives needed for batch onboarding. Status 2026-03-09: the current library does not yet cover likely next-batch structures such as `framework`, `processRail`, `principleChain`, `cards`, or `stages`, which will keep pushing authors back toward broad or duplicated primitives.
