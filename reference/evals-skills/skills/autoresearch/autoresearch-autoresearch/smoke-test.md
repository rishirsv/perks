---
name: autoresearch
description: "Run the execution phase of skill optimization after eval setup is complete. Autonomously optimize a skill by running it repeatedly, scoring outputs against binary evals, mutating the prompt, and keeping improvements. Use when: the target skill, test inputs, binary evals, run settings, and version name are already defined. Do NOT use for the full eval walkthrough or skill routing; use eval-orchestrator first when setup is incomplete. Outputs: an improved SKILL.md copy, a results log, and a changelog of every mutation tried."
---
...
This is the execution phase, not the full setup phase. If the eval harness is not ready yet, start with `eval-orchestrator` and come here only after the handoff contract is complete.
...
If the user instead says things like "set up evals," "walk me through the process," or "help me choose between error analysis, judge prompts, and validation," route them to `eval-orchestrator` first. That skill is responsible for the broader walkthrough and for producing these exact inputs.
...
## codex exec entrypoint

If the user wants to launch this from the command line with their Codex subscription, use:

- `../eval-orchestrator/scripts/codex_exec_runner.py` for the full orchestrated flow
- `scripts/run_autoresearch.py` as the thin convenience wrapper for direct skill-optimization runs

The runner is config-first. It reads a TOML config, applies CLI overrides such as model and reasoning effort, writes a run directory with the resolved config and generated prompt, then launches `codex exec`.
...
- `eval-orchestrator` when the user needs the setup walkthrough, routing, or Codex exec launch path
- A completed handoff contract: target skill, test inputs, binary evals, run settings, version name, and any evaluator artifacts

**What autoresearch feeds into:**
- The improved skill copy is ready for review and manual adoption
